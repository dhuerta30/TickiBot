...
Class PDOCrud {
    ...
    private function initializeBtnActionsSQL()
    {
        if ($this->settings["actionbtn"]) {
    ...
    /*
    limpia la condicion where para crear una busqueda personalizada en el callback
    */
   ...
   /**
    ...
    public function render($operationType = "CRUD", $data = array()) {
        ...
        if (strtoupper($operationType) === "ARRAY") {
            $data['source'] = 'array';
    ...
    private function dbSaveCrudData($data) {
        $data = $this->handleCallback('before_save_crud_data', $data);
        ...
        foreach ($updateData as $update) {
            $col = $update['col']; // Cambiado a array
            $pk_val = $update['id']; // Cambiado a array
            $val = $update['val']; // 
...
//        unset($this->btnActions["edit"]);
    ...
    private function dbCRUD($data) {
        ...
        /*if (isset($data['source']) && $data['source'] === 'array') {
            ...
            $dataArray = count($arrayData);

            $result = array(
                array("totalrecords" => $dataArray)
            );
            ...
            //echo $pdoModelObj->getLastQuery() . "<br>";
            ...
            if ($totalRecords > 0) {
                $pdoModelObj = $this->addJoinCondtion($pdoModelObj, false);
                if(isset($data["action"]) && $data["action"] != "pagination"){
                    $pdoModelObj = $this->addWhereCondition($pdoModelObj, $data);
                }
    ...
    private function dbSQL($data) {
        ...
        $result = $pdoModelObj->executeQuery("{$this->paginationQuery}", $data);
        $totalRecords = $result[0]["totalrecords"];
        $recordPerPage = $this->settings["recordsPerPage"];
        ...
        $output = $this->pdocrudView->renderSQL($cols, $search, $result, $pk, $this->objKey, $this->langData, $modal, $this->settings, $this->colsRemove, $pagination, $perPageRecords, $this->btnActions, $extraData);
    ...
    private function addGroupBy(PDOModel $pdoModelObj, $data = array(), $recordPerPage = 10) {
        ...
        if (isset($data["groupby"])) {
            ...
            $pdoModelObj->groupByCols = array($fieldName);
        }
            ...
            if (is_array($this->groupByCols)) {
                $pdoModelObj->groupByCols = $this->groupByCols;
            } else {
                $pdoModelObj->groupByCols = explode(',', $this->groupByCols);
            }
        } 
        // Si no se especifica un GROUP BY, se ignora
        else {
            $pdoModelObj->groupByCols = array();
        }
    ...
    public function getSearchBoxJSON($columns, $data = array()) {
        $searchContent = "";
        $searchContent2 = "";

        // Contenedor del select y configuración inicial
        $search = "<div class=\"col-md-5 col-sm-12 col-xs-12 pdo-search-cols form-group\">";
        $search .= "<select class=\"form-control pdocrud-form-control pdocrud_search_cols\">";

        // Opción para buscar en todas las columnas
        if (isset($this->settings["showAllSearch"]) && $this->settings["showAllSearch"]) {
            $search .= "<option value=\"all\">" . $this->langData["all"] . "</option>";
        }

        // Verificar si las columnas están definidas y tienen elementos
        if (is_array($columns) && count($columns)) {
            foreach ($columns as $key => $value) {
                // Soporte para columnas en formato simple (string) y extendido (array)
                if (is_string($value)) {
                    $colData = [
                        "col" => $value,
                        "colname" => ucfirst($value), // Formato amigable (capitalizado)
                        "type" => "text" // Tipo por defecto
                    ];
                } elseif (is_array($value) && isset($value["col"], $value["colname"], $value["type"])) {
                    $colData = $value;
                } else {
                    continue; // Ignorar columnas mal formateadas
                }

                // Verificar si la columna está en la lista de búsqueda permitida
                if (isset($this->searchBoxCols) && count($this->searchBoxCols)) {
                    if (!in_array($colData["col"], $this->searchBoxCols)) {
                        continue;
                    }
                }

                // Determinar si la columna está seleccionada
                $selected = "";
                if (isset($data["search_col"]) && $data["search_col"] === $colData["col"]) {
                    $selected = "selected=\"selected\"";
                }

                // Agregar la opción al select
                $search .= "<option $selected value=\"{$colData["col"]}\" data-type=\"{$colData["type"]}\">{$colData["colname"]}</option>";
            }
        }
        $search .= "</select></div>";
        ...
        if (isset($data["search_text"])) {
            $searchContent = array($data["search_text"]);
        }
        $search .= $this->getInputField(
            "pdocrud_search_box",
            ["placeholder" => $this->getLangData("search")],
            $searchContent,
            "text",
        ...
        // Campo de entrada para el texto adicional (rango de búsqueda)
        $searchTextToClass = "pdocrud-hide";
        if (isset($data["search_text2"])) {
            $searchContent2 = array($data["search_text2"]);
            $searchTextToClass = "";
        }
        $search .= $this->getInputField(
            "pdocrud_search_box_to",
            ["placeholder" => $this->getLangData("to")],
            $searchContent2,
            "text",
            ["pdocrud_search_input", $searchTextToClass]
        ...
        // Botón de búsqueda
        $class = $this->settings["template"] === "pure" ? "btn btn-primary" : "";
        $search .= "</div>";
        $search .= "<div class=\"col-md-1 col-2 col-xs-1 pdo-search-cols no-padding\">";
