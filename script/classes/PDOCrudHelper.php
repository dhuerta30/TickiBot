...
Class PDOCrudHelper {
    ...
    public function paginationJSON($page = 1, $totalrecords = 0, $limit = 10, $adjacents = 1, $lang = array()) {
        // Iniciar el array de paginación
        $pagination = [];
    
        // Si hay registros, continuar con la paginación
        if ($totalrecords > 0) {
            ...
            $lastpage = ceil($totalrecords / $limit);
            $lpm1 = $lastpage - 1; // Penúltima página
    
            // Inicializar el array de paginación
            $pagination = [
                'current_page' => $page,
                ...
                'pages' => []  // Array para las páginas
            ];
    
            // Si hay más de una página, generar los números de página
            if ($lastpage > 1) {
                ...
                if ($lastpage < 7 + ($adjacents * 2)) {
                    for ($counter = 1; $counter <= $lastpage; $counter++) {
                        $pagination['pages'][] = [
                            'page' => $counter,
                            'active' => ($counter == $page) ? true : false
                        ];
                    }
                } elseif ($lastpage >= 7 + ($adjacents * 2)) { // Si hay más de 7 páginas
                    if ($page < 1 + ($adjacents * 3)) { // Si la página actual está cerca del inicio
                        for ($counter = 1; $counter < 4 + ($adjacents * 2); $counter++) {
                            $pagination['pages'][] = [
                                'page' => $counter,
                                'active' => ($counter == $page) ? true : false
                        ...
                        $pagination['pages'][] = ['page' => '...', 'active' => false];
                        $pagination['pages'][] = ['page' => $lpm1, 'active' => false];
                        $pagination['pages'][] = ['page' => $lastpage, 'active' => false];
                    } elseif ($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)) { // Si la página está en el medio
                        $pagination['pages'][] = ['page' => 1, 'active' => false];
                        $pagination['pages'][] = ['page' => 2, 'active' => false];
                        $pagination['pages'][] = ['page' => '...', 'active' => false];
                        ...
                        $pagination['pages'][] = ['page' => $lpm1, 'active' => false];
                        $pagination['pages'][] = ['page' => $lastpage, 'active' => false];
                    } else { // Si la página está cerca del final
                        $pagination['pages'][] = ['page' => 1, 'active' => false];
                        $pagination['pages'][] = ['page' => 2, 'active' => false];
                        $pagination['pages'][] = ['page' => '...', 'active' => false];
